...
public:
	...
	simh_tape_file(const std::string_view path, const bool create = false);
	virtual ~simh_tape_file();

	// position-preserving operations
	virtual const bool is_read_only() const override { return m_read_only; }
	virtual const bool is_ew() const override { return m_pos + 32768 >= m_buf_len; } // 32KB from EOM; TODO: ANSI says EW should be 10ft from EOM regardless of density
	...
	virtual const std::pair<const tape_status, const u32> read_block(u8 *buf, const u32 buf_size) override;
...
protected:
	// internal operations
	void sync(const u32 len) const;

	// state
	u32 m_page_size; // size of system memory page
	int m_fd; // tape image file descriptor
	u8 *m_buf; // memory-mapped tape image file
	u32 m_buf_len; // size of tape image file
	bool m_read_only; // should we disallow destructive operations on tape image
	u32 m_pos; // tape position
};
